...
export default function UpdateClient({ params }) {
    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
            ...
            const NewValues = CompareValues(values, clients);
            // console.log("New values after comparison:", NewValues);  // Debug log
            ...
            if (NewValues) {
                dispatch(UpdateClients({
                    ...
                    console.log("Response after dispatch:", res);  // Debug log
                    if (res?.meta?.requestStatus === "fulfilled") {
                        toast.success("Client Updated", {
                            action: {
                                label: "Close",
                    ...
                    console.log("Dispatch error:", err);  // Debug log for errors
                });
            } else {
                toast.warning("No Changes", {
                    action: {
    ...
    useEffect(() => {
        dispatch(GetSpecificClient(ClientID)).then((client) => {
            ...
            console.log("Error fetching specific client:", err);  // Debug log for fetching errors
        });
    }, []);
    ...
    return (
        <section className="flex gap-2 flex-col bg-white rounded-md shadow-md my-4 p-3 md:p-8">
            ...
            <form onSubmit={formik.handleSubmit}>
                <main className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                    {updateClientInputs.map((input, indx) => (
                        <InputDemo
                            ...
                            style={input.style}
                            onChange={
                                input.type === "file" ? (e) => formik.setFieldValue(input.id, e.target.files[0])
                                : input.type === "selected" ? (e) => formik.setFieldValue(input.id, e)
                                : formik.handleChange
                            }
                            value={formik.values[input.id]}
                            type={input.type}
                ...
                <div className="w-full flex items-center justify-end gap-7 my-8" variant="outline">
                    <Link href={`/clients`} className={`${buttonVariants({ variant: "outline" })} text-red-500 hover:bg-red-300`}>
                        Cancel
                    </Link>
                    <Button type="submit" className="bg-main-100" disabled={loading}>
                        {loading ? "Loading..." : "Save client"}
                    </Button>
                </div>
            </form>
