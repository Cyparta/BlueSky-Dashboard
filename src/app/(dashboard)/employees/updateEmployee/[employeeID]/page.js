...
const UpdateEmployeePage = ({params}) => {
    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
            if (values.user_permissions.length === 0) {
                ...
                const NewValues = deepCompareValues(values, employeeData);
    ...
    useEffect(()=>{
        dispatch(GetSpecificEmployee(params.employeeID)).then((result)=>{
            ...
            formik.setFieldValue("first_name", result?.payload.user.first_name);
            formik.setFieldValue("phone", result?.payload.user.phone);
